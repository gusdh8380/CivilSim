...
### 2-3. 건물 UI 패널
- [x] `BuildingPanelUI.cs` — 하단 건물 선택 패널 (카테고리 탭, B키 토글)
- [x] `BuildingButtonUI.cs` — 건물 버튼 (호버 하이라이트, 선택 유지)
- [x] `BuildingTooltipUI.cs` — 마우스 추적 툴팁 (비용, 크기, 인구, 설명)
- [x] `PlacedBuildingSelector.cs` — 배치 건물 클릭 선택 + 노란 하이라이트
- [x] `SelectedBuildingPanel.cs` — 선택 건물 정보 패널 + 철거 버튼
